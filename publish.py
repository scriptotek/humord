...
def run():
    ...
    files = [
        {
            'remote_url': 'http://www.bibsys.no/files/out/humordsok/HUMEregister.xml',
            'local_file': 'src/humord.xml'
        },{
...
def make():
    ...
    roald.set_uri_format('http://data.ub.uio.no/humord/c{id}')
    roald.save('humord.json')
    ...
    roald.export('dist/humord.marc21.xml', format='marc21', **marc21options)
    roald.export('dist/humord.ttl', format='rdfskos',
             include=['humord.scheme.ttl', 'ubo-onto.ttl'],
             mappings_from=['src/lambda.rdf'])
